...
from vog_optimizer.utils import dataset_with_indices


class LittleMnistModel(pl.LightningModule):
    def __init__(self, data_dir='./', learning_rate=2e-4):
        super(LittleMnistModel, self).__init__()
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            mnist_fit = mnist_with_indices(self.data_dir, train=True, transform=self.transform)
            self.mnist_train, self.mnist_val = random_split(mnist_fit, [55000, 5000])

        if stage == 'test':
            mnist_with_indices = dataset_with_indices(MNIST)
            self.mnist_test = mnist_with_indices(self.data_dir, train=False, transform=self.transform)
...
class BigMnistModel(pl.LightningModule):
